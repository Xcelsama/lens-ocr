...
async function createOcrWorker(progress) {
  ...
  // tesseract.js reports its own sub-steps (loading core, loading language,
  // initializing api, etc.) via this logger. Surfacing it lets us see
  // exactly which internal step is hanging, instead of just "still creating
  // the worker somewhere".
  const logger = progress
    ? (m) => {
        progress.stage = `worker-init:${m.status}${
  ...
  try {
    ...
    if (progress) progress.stage = `worker-init:eng+osd-failed(${err.message})`;
...
// one worker.
async function acquireWorker(progress) {
...
export async function runOcr(inputBuffer, progress = {}) {
  ...
  const entry = await acquireWorker(progress);
