...
async function createOcrWorker() {
  ...
  try {
    ...
    console.log(`[ocr] worker init (eng+osd) took ${Date.now() - start}ms`);
    return worker;
  } catch (err) {
    osdAvailable = false;
    ...
    await worker.setParameters({ preserve_interword_spaces: '1' });
    console.log(`[ocr] worker init (eng only, fallback) took ${Date.now() - fallbackStart}ms`);
    return worker;
  }
}
...
export async function runOcr(inputBuffer) {
  ...
  try {
    ...
    console.log(
      `[ocr] timings(ms) worker=${tWorkerReady - t0} preprocess=${tPreprocessed - tWorkerReady} ` +
        `rotation=${tRotationChecked - tPreprocessed} recognize=${tRecognized - tRotationChecked} ` +
        `total=${tRecognized - t0}`
    );
